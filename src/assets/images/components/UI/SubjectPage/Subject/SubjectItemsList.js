...
function SubjectItemsList(props) {
  ...
  const handleChange = (event, newValue) => {
    // if ((newValue !== 1 && props.path.includes('quiz')) || (newValue !== 2 && props.path.includes('ppts'))) {
    //   history.replace(props.path.split(props.path.includes('quiz') ? '/quiz' : '/ppts')[0])
    // } else if (newValue !== 3 && props.path.includes("notes")) {
    //   history.replace(props.path.split('/notes')[0])
    // }
  ...
  const tabpanels = [
    ...
    <PPTs
      ...
      endPointValue={props.course_id}
    />,
    {/* <Notes TODO add notes only here.
      path={props.path}
      subscription={props.subscription}
      notes={props.notes}
      endPointValue={props.course_id}
